...
%           range: 32-bit integer (int32), len=6 
%     donor_range: 32-bit integer (int32), len=6 
...
if nargin<7
    io_range=zeros(1,6,'int32');
elseif length(io_range)<6
    % Enlarge the array if necessary;
    if size(io_range,2)==1;
        io_range=[io_range; zeros(6-length(io_range),1,'int32')];
    else
        io_range=[io_range, zeros(1,6-length(io_range),'int32')];
    end
elseif ~isa(io_range,'int32');
...
if nargin<8
    io_donor_range=zeros(1,6,'int32');
elseif length(io_donor_range)<6
    % Enlarge the array if necessary;
    if size(io_donor_range,2)==1;
        io_donor_range=[io_donor_range; zeros(6-length(io_donor_range),1,'int32')];
    else
        io_donor_range=[io_donor_range, zeros(1,6-length(io_donor_range),'int32')];
    end
elseif ~isa(io_donor_range,'int32');
...
if nargin<9
    io_transform=zeros(1,3,'int32');
elseif length(io_transform)<3
    % Enlarge the array if necessary;
    if size(io_transform,2)==1;
        io_transform=[io_transform; zeros(3-length(io_transform),1,'int32')];
    else
        io_transform=[io_transform, zeros(1,3-length(io_transform),'int32')];
    end
elseif ~isa(io_transform,'int32');
...
[ierr, io_connectname, io_donorname] =  cgnslib_mex(int32(103), in_fn, in_B, in_Z, in_I, io_connectname, io_donorname, io_range, io_donor_range, io_transform);
