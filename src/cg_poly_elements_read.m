function [io_elements, io_connect_offset, io_parent_data, ierr] = cg_poly_elements_read(in_file_number, in_B, in_Z, in_S, io_elements, io_connect_offset, io_parent_data)
% Gateway function for C function cg_poly_elements_read.
%
% [elements, connect_offset, parent_data, ierr] = cg_poly_elements_read(file_number, B, Z, S, elements, connect_offset, parent_data)
%
...
%    connect_offset: 64-bit integer (int64), array  (must be preallocated at input)
...
basetype = 'int64';
if ~isa(io_connect_offset,basetype)
    io_connect_offset = cast(io_connect_offset, basetype);
elseif ~isempty(io_connect_offset)
    % Write to it to unshare memory with other variables
...
ierr = cgnslib_mex(int32(97), in_file_number, in_B, in_Z, in_S, io_elements, io_connect_offset, io_parent_data);
