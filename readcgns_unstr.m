...
% Read file in CGNS format via CGNS mid-library.
%
% [xs, elems, typestr, var_nodes, var_cells] = READCGNS_UNSTR( filename, node_var_list, elem_var_list).
%
% Input arguments:
...
%     [xs, elems, etype, tmp, var_cells] = writecgns_unstr( 'test.cgns', '-');
%
...
% Open the CGNS file
[index_file,ierr] = cg_open(filename, CG_MODE_READ);
...
[sectionname,itype,istart,iend, nbndry, pflag, ierr] = cg_section_read(index_file, ...
    index_base,index_zone,index_sect,sectionname); chk_error(ierr);
...
% "test readcgns_unstr" in Octave or "test_mcode readcgns_unstr" in MATLAB.
...
%! if(~isequal(xs,xs1) || ~isequal(elems , elems1))
...
%! if(~isequal(xs,xs1) || ~isequal(elems, elems1) || ~isequal(nodal_vars,var_nodes))
...
%! if(~isequal(xs,xs1) || ~isequal(elems, elems1) || ~isequal(eleml_vars,var_cells))
...
%! if(~isequal(xs,xs1) || ~isequal(elems,elems1) || ~isequal(nodal_vars,var_nodes) || ~isequal(eleml_vars,var_cells))
%!     error('Error: incorrectly read nodal variables from mixed mesh');
%! end
