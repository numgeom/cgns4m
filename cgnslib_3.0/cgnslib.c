...
static int adf2_update_elems( int *elems, int datasize, int dir) {
    ...
    while (i < datasize) {
        int etype = (ElementType_t)elems[i];
        if (dir==-1 && etype == PYRA_13 || etype == NFACE_n) {
            cgi_error("PYRA_13 and NFACE_n are not supported in ADF2.");
            return CG_ERROR;
        }
        else if (etype >= PYRA_13)
            elems[i] = elems[i] + dir;
...
int cg_section_write(int file_number, int B, int Z, char const * SectionName, ElementType_t type,
    ...
    if ( cgns_filetype==CG_FILE_ADF2 && type >= PYRA_13) {
        /* Jiao: Changed to use older compatible version */
        if (type == PYRA_13 || type == NFACE_n) {
            cgi_error("PYRA_13 and NFACE_n are not supported in ADF2.");
            return CG_ERROR;
        }
    ...
    if ( cgns_filetype==CG_FILE_ADF2 && type == MIXED) {
