...
You can generate CGNS4m for a different version of `cgnslib`, copy its source code into the `cgnslib_new_version` directory, and merge the annotation in `cgnslib_3.4.1/cgnslib.h` into `cgnslib_new_version/cgnslib.h`. Finally, modify `mexUtil/codegen.sh` and `mexUtil/build_cgns4m.sh` to use this new cgnslib directory, and run `codegen.sh` in `bash` and `build_cgns4m` in MATLAB/Octave. Note that CGNS4m does not use the build process of `cgnslib`, so you can safely remove its cmake and makefiles.
