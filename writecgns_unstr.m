function writecgns_unstr( file_name, ps, elems, typestr, var_nodes, var_cells)
...
% Open the CGNS file.
[index_file,ierr] = cg_open(file_name,CG_MODE_WRITE);
if ierr
    error( ['Could not open file ' file_name ' for output.']);
end
...
isize = [size(ps,1), number_of_elements(elems), zeros(1,7)];
% Create zone
...
if type == MIXED
    elems = convert_mixed_elements( elems, str2double(typestr(end)));
end
[index_sec, ierr] = cg_section_write(index_file,index_base,index_zone,'Elements', ...
    type, 1, number_of_elements( elems), 0, elems'); chk_error(ierr);
...
function [type, icelldim] = get_elemtype( npe, typestr)
...
switch (npe)
    case 1
        ...
        elseif strcmpi(typestr,'MIXED3')
            ...
            error('For mixed meshes, typestr must be MIXED2 or MIXED3.');
        end
    case 2
        type = BAR_2;
        icelldim = 1;
...
function elems = convert_mixed_elements( elems, dim)
...
% "test writecgns_unstr" in Octave or "test_mcode writecgns_unstr" in MATLAB.
