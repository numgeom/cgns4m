...
int VersionList[] = {2540, 2530, 2520, 2510, 2500, 2460, 2420, 2400,
...
int cg_close(int file_number) {
    ...
    if (cgns_compress && cg->mode == CG_MODE_MODIFY &&
       (cg->deleted || cgns_compress == 1)) {
        char *cg_filename = cg->filename;
        char *temp_filename;
        double output_id;
#ifdef S_IFLNK
        struct stat st;
        if (!lstat (cg->filename, &st) && (st.st_mode & S_IFLNK) == S_IFLNK) {
            int len;
            cg_filename = CGNS_NEW(char, st.st_size + 1);
            len = readlink (cg->filename, cg_filename, st.st_size + 1);
            if (len < 0 || len > st.st_size) {
                CGNS_FREE (cg_filename);
                cg_filename = cg->filename;
            }
            else {
                cg_filename[len] = 0;
            } 
...
#endif
        temp_filename = CGNS_NEW(char, strlen(cg_filename)+6);
        sprintf(temp_filename,"%s.temp",cg_filename);
        unlink(temp_filename);

        ADF_Database_Open (temp_filename, "NEW", "NATIVE", &output_id, &ierr);
        if (ierr > 0) {
            CGNS_FREE (temp_filename);
            if (cg_filename != cg->filename) CGNS_FREE (cg_filename);
            adf_error ("ADF_Database_Open", ierr);
            return CG_ERROR;
        }
     ...
     /* adf_cond copies & compressed data from input_id to output_id, then
        ...
        unlink (cg_filename);
        if (rename (temp_filename, cg_filename)) {
            cgi_error("rename of %s -> %s failed", temp_filename, cg_filename);
            return CG_ERROR;
        }
        CGNS_FREE (temp_filename);
        if (cg_filename != cg->filename) CGNS_FREE (cg_filename);
...
int cg_configure(int what, void *value) {
    if (what == CG_CONFIG_ERROR)
        ...
        ADF_Search_Delete(&ierr);
        if (path && *path) {
            ADF_Search_Add(path, &ierr);
            if (ierr > 0) {
                adf_error("ADF_Search_Add",ierr);
                return CG_ERROR;
            }
        }
    }
        ...
        int ierr;
        ADF_Search_Add((char *)value, &ierr);
        if (ierr > 0) {
            adf_error("ADF_Search_Add",ierr);
            return CG_ERROR;
        }
...
int cg_is_cgns(const char *filename) {
    ...
    if (access (filename, 0) || stat (filename, &st) ||
        S_IFREG != (st.st_mode & S_IFREG)) return CG_NODE_NOT_FOUND;
    ADF_Database_Valid(filename, &ierr);
...
*/
    ...
    ADF_Read_All_Data(coord->id, xyz, &ierr);
    if (ierr>0) {
...
static int element_data_size(ElementType_t type, int nelems, const int *connect) {
    int npe, size;

    if (type < MIXED) {
        if (cg_npe(type, &npe)) return -1;
        if (npe <= 0) {
...
int cg_elements_read(int file_number, int B, int Z, int S, int *elements,
    ...
    count = element_data_size(section->el_type, num, section->connect->data);
    ...
    if (section->connect->data) {
        ...
        ADF_Read_All_Data(section->connect->id, (char *)elements, &ierr);
        if (ierr > 0) {
            adf_error("ADF_Read_All_Data", ierr);
            return CG_ERROR;
        }
    ...
    if (section->parent && parent_data) {
        if (section->parent->data) {
            ...
            ADF_Read_All_Data(section->parent->id, (char *)parent_data, &ierr);
            if (ierr > 0) {
                adf_error("ADF_Read_All_Data", ierr);
                return CG_ERROR;
            }
...
int cg_field_read(int file_number, int B, int Z, int S, char const *fieldname, DataType_t type,
...
*/
    ...
    ADF_Read_All_Data(field->id, values, &ierr);
    if (ierr>0) {
        adf_error("ADF_Read_All_Data",ierr);
        return CG_ERROR;
    }
    ...
    for (set=0; set<hole->nptsets; set++) {
        if (hole->ptset[set].npts>0) {
            ADF_Read_All_Data(hole->ptset[set].id,
                (void *)((int *)pnts+2*index_dim*set), &ierr);
            if (ierr>0) {
                adf_error("ADF_Read_All_Data",ierr);
                return CG_ERROR;
            }
...
int cg_conn_read(int file_number, int B, int Z, int I, int *pnts,
    ...
    if (conn->ptset.npts>0) {
        ADF_Read_All_Data(conn->ptset.id, (void *)pnts, &ierr);
        if (ierr>0) {
            adf_error("ADF_Read_All_Data",ierr);
            return CG_ERROR;
        }
    ...
    if (conn->dptset.npts>0) {
        ...
        ADF_Read_All_Data(dptset.id, dpnts, &ierr);
        if (ierr>0) {
            adf_error("ADF_Read_All_Data",ierr);
            return CG_ERROR;
        }
 ...
 */
    ...
    if (one21->ptset.npts > 0) {
        ADF_Read_All_Data(one21->ptset.id, (void *)range, &ierr);
        if (ierr>0) {
            adf_error("ADF_Read_All_Data",ierr);
            return CG_ERROR;
        }
    ...
    if (one21->dptset.npts > 0) {
        ADF_Read_All_Data(one21->dptset.id, (void *)donor_range, &ierr);
        if (ierr>0) {
            adf_error("ADF_Read_All_Data",ierr);
            return CG_ERROR;
        }
...
int cg_boco_read(int file_number, int B, int Z, int BC, int *pnts, void *NormalList) {
    ...
    if (boco->ptset && boco->ptset->npts>0) {
        ADF_Read_All_Data(boco->ptset->id, (void *)pnts, &ierr);
        if (ierr>0) {
            adf_error("ADF_Read_All_Data",ierr);
            return CG_ERROR;
        }
...
int cg_array_read(int A, void *Data) {
    ...
    if (array->data)
        ...
        ADF_Read_All_Data(array->id, Data, &ierr);
        if (ierr>0) {
            adf_error("ADF_Read_All_Data",ierr);
            return CG_ERROR;
        }
...
int cg_array_read_as(int A, DataType_t type, void *Data) {
    ...
    if (array->data)
        ...
        ADF_Read_All_Data(array->id, array_data, &ierr);
        if (ierr>0) {
            adf_error("ADF_Read_All_Data",ierr);
            return CG_ERROR;
        }
...
/* begin KMW
    if(exponent->nexps == 8)
	cgi_warning("Full set of exponents written, may want to use cg_expfull_read.");
   end KMW */
...
int cg_ptset_read(int *pnts)
{
    ...
    if (ptset->npts > 0)
    {
        ADF_Read_All_Data(ptset->id, (void *)pnts, &ier);

        if (ier>0)
	{
            adf_error("ADF_Read_All_Data",ier);
            return CG_ERROR;
        }
/*
    } else
    ...
    for (index=0; index<zcoor->ncoords; index++) {
        if (strcmp(coordname, zcoor->coord[index].name)==0) {
            ...
            if (type==cgi_datatype(coord->data_type)) {
                ADF_Write_All_Data(coord->id, coord_ptr, &ierr);
                if (ierr>0) {
                    adf_error("ADF_Write_All_Data", ierr);
                    return CG_ERROR;
                }
...
int cg_section_write(int file_number, int B, int Z, char const * SectionName, ElementType_t type,
    ...
    num = end - start +1;
    ElementDataSize = element_data_size(type, num, elements);
    ...
    if (section->connect->data == NULL) {
        cgi_error("Error allocating element data");
        return CG_ERROR;
    }
...
#ifndef ELEMENTS_IN_MEMORY
    ADF_Write_All_Data(section->connect->id, (const char *)elements, &ierr);
    if (ierr > 0) {
        adf_error("ADF_Write_All_Data", ierr);
        return CG_ERROR;
    }
...
#ifndef ELEMENTS_IN_MEMORY
    ADF_Write_All_Data(section->parent->id, (const char *)parent_data, &ierr);
    if (ierr > 0) {
        adf_error("ADF_Write_All_Data", ierr);
        return CG_ERROR;
    }
    ...
    for (index=0; index<sol->nfields; index++) {
        if (strcmp(fieldname, sol->field[index].name)==0) {
            ...
            if (type==cgi_datatype(field->data_type)) {
                ADF_Write_All_Data(field->id, field_ptr, &ierr);
                if (ierr>0) {
                    adf_error("ADF_Write_All_Data", ierr);
                    return CG_ERROR;
                }
...
int cg_field_partial_write(int file_number, int B, int Z, int S,
    ...
    cgns_array *field;
    int n, ierr = 0;
    int index, index_dim, dims[ADF_MAX_DIMENSIONS];
    ...
    for (index=0; index<sol->nfields; index++) {
        if (strcmp(fieldname, sol->field[index].name)==0) {
            int m_start[ADF_MAX_DIMENSIONS], m_end[ADF_MAX_DIMENSIONS];
            int m_dim[ADF_MAX_DIMENSIONS], stride[ADF_MAX_DIMENSIONS];
            ...
            ADF_Write_Data(field->id, rmin, rmax, stride,
                           field->data_dim, m_dim, m_start, m_end,
                           stride, (void *)field_ptr, &ierr);
...
int cg_coord_partial_write(int file_number, int B, int Z, DataType_t type,
    ...
    for (index=0; index<zcoor->ncoords; index++) {
        if (strcmp(coordname, zcoor->coord[index].name)==0) {
            ...
            ADF_Write_Data(coord->id, rmin, rmax, stride,
                           coord->data_dim, m_dim, m_start, m_end,
                           stride, (void *)coord_ptr, &ierr);
...
int cg_section_partial_write(int file_number, int B, int Z,
			     ...
			     int const * elements, int *S) {
    cgns_zone *zone;
    cgns_section *section;
    int index, ElementDataSize;
    ...
    ElementDataSize = element_data_size(type, end-start+1, elements);
    if (ElementDataSize < 0) return CG_ERROR;
    ...
    for (index=0; index<zone->nsections; index++) {
        if (strcmp(SectionName, zone->section[index].name)==0) {
            int i, n, size, num, offset, ierr;
            int oldsize, newsize, elemsize;
            int *oldelems, *newelems;
            double id;

            section = &(zone->section[index]);

            /* must have the existing and new element lists of the same type */
            if(type != section->el_type) {
                cgi_error("Mismatch in element types for section '%s'",SectionName);
                return CG_ERROR;
            }
            ...
            if (end < section->range[0]) {
                ...
                if (end < section->range[1]) {
                    ...
                    offset = element_data_size(type, num, oldelems);
            ...
            if (start <= section->range[0]) {
                ...
                size = element_data_size(type, num, oldelems);
            ...
            ADF_Get_Node_ID(section->id, "ElementRange", &id, &ierr);
            if (ierr > 0) {
                adf_error("ADF_Get_Node_ID", ierr);
                return CG_ERROR;
            }
            ADF_Write_All_Data(id, (char *)section->range, &ierr);
            if (ierr > 0) {
                adf_error("ADF_Write_All_Data", ierr);
                return CG_ERROR;
            }
            ...
            ADF_Put_Dimension_Information(section->connect->id,
                section->connect->data_type, 1,
                section->connect->dim_vals, &ierr);
            if (ierr > 0) {
                adf_error("ADF_Put_Dimension_Information", ierr);
                return CG_ERROR;
            }
            ADF_Write_All_Data(section->connect->id, (char *)newelems, &ierr);
            if (ierr > 0) {
                adf_error("ADF_Write_All_Data", ierr);
                return CG_ERROR;
            }
            ...
            if (section->parent) {
                ...
                ADF_Put_Dimension_Information(section->parent->id,
                    section->parent->data_type, 2,
                    section->parent->dim_vals, &ierr);
                if (ierr > 0) {
                    adf_error("ADF_Put_Dimension_Information", ierr);
                    return CG_ERROR;
                }
                ADF_Write_All_Data(section->parent->id, (char *)newelems, &ierr);
                if (ierr > 0) {
                    adf_error("ADF_Write_All_Data", ierr);
                    return CG_ERROR;
                }
    ...
    section->connect->data = (void *)malloc(ElementDataSize*sizeof(int));
    if (section->connect->data == NULL) {
        cgi_error("Error allocationg element data");
        return CG_ERROR;
    }
    memcpy(section->connect->data, elements, ElementDataSize*sizeof(int));
    strcpy(section->connect->name,"ElementConnectivity");
...
int cg_parent_data_partial_write(int file_number, int B, int Z, int S,
    ...
    if (parent) {
        ...
        ADF_Write_All_Data(parent->id, (char *)data, &ierr);
        if (ierr > 0) {
            adf_error("ADF_Write_All_Data", ierr);
            return CG_ERROR;
        }
...
int cg_elements_partial_read(int file_number, int B, int Z, int S,
    ...
    */
    ...
    offset = element_data_size(section->el_type,
                 start - section->range[0], data);
    size = element_data_size(section->el_type,
...
int cg_ElementPartialSize(int file_number, int B, int Z, int S,
    ...
    if (section->el_type < MIXED) {
        size = element_data_size(section->el_type, end - start + 1, 0);
        if (size < 0) return CG_ERROR;
